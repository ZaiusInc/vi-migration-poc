...
import {logger, notifications, ValueHash} from '@zaius/app-sdk';
  ...
  public async perform(status: HistoricalImportJobStatus): Promise<HistoricalImportJobStatus> {
    ...
    if (encounteredError) {
      if (state.retries >= 5) {
        ...
        await notifications.error('Historical Import', 'Failed to complete historical import', 'Maximum retries exceeded');
        status.complete = true;
      } else {
        state.retries++;
